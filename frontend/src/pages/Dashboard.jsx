...
import {
  TrendingUp, Activity, Brain, Users,
  ArrowRight, Sparkles, Target,
} from 'lucide-react';
import { fetchDashboardStats, fetchEmployees } from '../api';
...
export default function Dashboard() {
  ...
  useEffect(() => {
    Promise.all([fetchDashboardStats(), fetchEmployees()])
      .then(([s, e]) => { setStats(s); setEmployees(e); })
      .catch(console.error)
      .finally(() => setLoading(false));
  }, []);

  if (loading) return <Loader />;
  if (!stats) return <p className="text-surface-500 text-center py-20">Failed to load dashboard.</p>;
...
function StatCard({ icon: Icon, label, value, sub, colorClass, iconBg, iconColor }) {
  return (

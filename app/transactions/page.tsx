...
import { fetchEmployees, analyzeProfile } from "@/lib/api";
...
interface AnalysisResult {
  ...
  transformational_learning_score: number;
  emotional_recovery_score: number;
...
export default function Transactions() {
  ...
  const handleAnalyze = async (empId: number) => {
    setAnalyzing((p) => ({ ...p, [empId]: true }));
    try {
      const result = await analyzeProfile(empId);
      setAnalyses((p) => ({ ...p, [empId]: result }));
      setExpanded((p) => ({ ...p, [empId]: true }));
    } catch (err) {
      console.error(err);
    } finally {
      setAnalyzing((p) => ({ ...p, [empId]: false }));
    }
  };
                ...
                <div className="animate-fade-in border-t border-surface-100 p-6 space-y-6 bg-surface-50/50">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 gap-2.5 sm:grid-cols-2">
                      {Object.entries(a.feature_importance)
                        .slice(0, 8)
                        .map(([key, val]) => (

...
const getColor = (domain: string) =>
  DOMAIN_COLORS[domain] ?? { fill: "#6366f1", light: "#eef2ff" };
...
function buildGraph(transitions: Transition[], employees: Employee[]) {
  const nodes: GraphNode[] = employees.map((emp) => ({
    id: emp.id,
    name: emp.name,
    initials: emp.name
      .split(" ")
      ...
      for (let j = i + 1; j < domainEmps.length; j++) {
        ...
        if (srcIdx >= 0 && tgtIdx >= 0) {
          if (
            !edges.some(
              (e) =>
                (e.source === srcIdx && e.target === tgtIdx) ||
                (e.source === tgtIdx && e.target === srcIdx),
            )
          ) {
            edges.push({
              source: srcIdx,
...
export default function KnowledgeGraph({ transitions = [], employees = [] }: Props) {
  ...
  const handleNodeEnter = useCallback((node: GraphNode, pos: { x: number; y: number }) => {
    ...
    <div ref={containerRef} className="relative w-full">
      ...
      >
        ...
        {edges.map((edge, i) => {
          ...
          const isHighlighted =
            hoveredNode === nodes[edge.source]?.id || hoveredNode === nodes[edge.target]?.id;

          return (
            <g
              key={`edge-${i}`}
              className="kg-edge animate-draw-line"
        ...
        {nodes.map((node, i) => {
          ...
          return (
            ...
            >
              {isHovered && (
                <circle cx={pos.x} cy={pos.y} r={radius + 8} fill={color.fill} opacity={0.12} />
              )}
              <circle
